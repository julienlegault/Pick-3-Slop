(function(global) {
  ...
  function drawBoons(count, gl) {
    ...
    var byRarity = { common: [], uncommon: [], rare: [], legendary: [] };
    for (var i = 0; i < BOONS.length; i++) byRarity[BOONS[i].rarity].push(BOONS[i]);

    var rarityW = rarityOrder.map(function(r) {
      var base = byRarity[r].reduce(function(s, b) { return s + b.w; }, 0);
      return base * rarityMult(r, gl);
    });
    var poolTotals = rarityOrder.reduce(function(acc, r) {
      acc[r] = byRarity[r].reduce(function(s, b) { return s + b.w; }, 0);
      return acc;
    }, {});
    var rarityTotal = rarityW.reduce(function(s, w) { return s + w; }, 0);
    if (rarityTotal <= 0) return [];

    var out = [];
    var shopStamp = Date.now();
    for (var n = 0; n < count; n++) {
      var rr = Math.random() * rarityTotal;
      var pickedRarity = rarityOrder[rarityOrder.length - 1];
      for (var rix = 0; rix < rarityOrder.length; rix++) {
        rr -= rarityW[rix];
        if (rr <= 0) {
          pickedRarity = rarityOrder[rix];
          break;
        }
      }

      var pool = byRarity[pickedRarity];
      var poolTotal = poolTotals[pickedRarity];
      var rb = Math.random() * poolTotal;
      var chosen = pool[pool.length - 1];
      for (var pix = 0; pix < pool.length; pix++) {
        rb -= pool[pix].w;
        if (rb <= 0) {
          chosen = pool[pix];
          break;
      ...
      out.push(Object.assign({}, chosen, { iid: chosen.id + '_' + shopStamp + '_' + n + '_' + Math.random().toString(36).slice(2, 8) }));
    }
    return out;
  }
